...
routes.post("/RemovesubToStudent",(req,res)=>{
    console.log(req.body.rollno,req.body.subject)
    let subs = req.body.subject; 
    student.collection(req.body.dept+"").updateOne(
        { Rollno: req.body.rollno },
            { $unset: { [`subjects.${req.body.subject}`]: 1 } },
        (error, result) => {
            if (error) {
                console.error(error);
                res.status(500).json({ error: "Internal Server Error" });
            ...
            console.log("Subject removed to student:", result);
...
routes.post("/fetchSubStudent",(req,res)=>{
    console.log(req.body.subject,req.body.dept)
    student.collection(req.body.dept+"").find({
        [`subjects.${req.body.subject}`]: { $exists: true }
      }).toArray((error, students) => {
        if (error) {
          console.error(error);
          res.status(500).json({ error: "Internal Server Error" });
          return;
        ...
        console.log(`Students who have ${req.body.subject}:`, students);
